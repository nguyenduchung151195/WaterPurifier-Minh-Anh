...
import settingsManager from '../../utils/call/settingsManager';
...
export default class Login extends React.Component {
        ...
        <div data-component="Login">
          ...
          <form action="" onSubmit={this.handleSubmit.bind(this)}>
            <div className="settings-icon-container">
              <SettingsIcon className="icon" color="#666" hoverColor="#333" onClick={this.handleClickSettings.bind(this)} />
            </div>
          ...
          <div className="submit-container">
            <div className={classnames('submit-button', { disabled: !this._checkCanPlay() })} onClick={this.handleSubmitClick.bind(this)} />
          </div>

          <If condition={state.showSettings}>
            <div className="settings-container">
              <Settings settings={settings} onSubmit={this.handleSettingsSubmit.bind(this)} onCancel={this.handleSettingsCancel.bind(this)} />
            </div>
          </If>
        </div>
      </TransitionAppear>
  ...
  _checkCanPlay() {
    ...
    if (state.settings.display_name && !state.showSettings) return true;
    return false;
  }
  ...
  _checkForm() {
    ...
    if (!settings.uri) {
      const username = `${settings.display_name.toLowerCase().replace(/[^a-zA-Z0-9]/g, '')}_${randomString({ length: 6 }).toLowerCase()}`;
