...
class ChartCard extends React.Component {
  ...
  render() {
    const { classes, chartColor, statIconColor, chart, title, text, statLink, statText, underChart, hover } = this.props;
    const cardHeaderClasses = `${classes.cardHeader} ${classes[`${chartColor}CardHeader`]}${cx({
      [` ${classes.moveChartUp}`]: this.state.hover && hover,
    })}`;
    const addHoverEvent = {};
    if (hover) {
      if (navigator.userAgent.match(/iPad/i) != null) {
        addHoverEvent.onClick = () => this.setState({ hover: !this.state.hover });
      } else {
    ...
    return (
      <Card className={classes.card} {...addHoverEvent}>
        ...
        <CardContent className={classes.cardContent}>
          {hover ? <div className={classes.underChart}>{underChart}</div> : null}
        ...
        {this.props.statIcon !== undefined || statLink !== undefined || statText !== undefined ? (
...
ChartCard.propTypes = {
  ...
  chartColor: PropTypes.oneOf(['orange', 'green', 'red', 'blue', 'purple', 'rose']),
