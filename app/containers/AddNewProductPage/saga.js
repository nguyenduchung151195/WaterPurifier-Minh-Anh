...
import {
  ...
  getGroupProductFailed,
} from './actions';

import { addProduct } from '../AddExpensesPage/actions';
import { GET_TAGS, ADD_NEW_PRODUCT, GET_GROUP_PRODUCT } from './constants';
import {
  API_TAG_STOCK,
  SUPPLIER,
  GET_PROP_SET,
...
export function* getGroupProduct() {
  try {
    const token = localStorage.getItem('token');
    const data = yield call(request, API_GROUP_PRODUCT, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    if (data) {
      yield put(getGroupProductSuccess(data));
    } else {
      yield put(getGroupProductFailed({}));
    }
  } catch (err) {
    yield put(getGroupProductFailed(err));
  }
}
...
export default function* addNewProductPageSaga() {
  ...
  yield takeEvery(GET_GROUP_PRODUCT, getGroupProduct);
