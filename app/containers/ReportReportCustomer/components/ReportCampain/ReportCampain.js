...
import { API_BY_CAMPAIN } from 'config/urlConfig';
...
function ReportCampain(props) {
  ...
  useEffect(
    () => {
      if (months) {
        fetchData(`${API_BY_CAMPAIN}?month=${month}&year=${year}&${serialize(filter)}`)
          .then(res => {
  ...
  const handleCloseExcel = () => {
    ...
    let title = 'Báo cáo theo chiến dịch';
    switch (type) {
      case 'PDF':
        // console.log('sss');
        const { totalPage = 1, pageNumber = 1 } = {};
        const content = tableToPDF('reportCampain');
        setHtml(e => [...e, { content, pageNumber }]);
        setHtmlTotal(totalPage);
        break;
      default:
        tableToExcelCustomNoList('reportCampain', 'W3C Example Table', undefined, undefined, title);
    }
  };
    ...
    <>
      <Paper style={{ width: props.miniActive ? window.innerWidth - 80 : window.innerWidth - 260 }}>
        <Grid className="helloCVDA" style={{ padding: '0 20px', width: props.miniActive ? 'calc( 100vw - 80px)' : 'calc( 100vw - 260px)' }}>
          <CustomAppBar title="BÁO CÁO THEO CHIẾN DỊCH" onGoBack={props.onClose} disableAdd />
          ...
          <Grid item md={12} container spacing={32} style={{ paddingTop: 30, paddingLeft: 50}}>
            ...
            <Grid item xs={2}><DepartmentAndEmployee onChange={handleChangeDepartment} disableEmployee moduleCode="Employee" profile={props.profile} fullWidth /></Grid>
            <Grid item  xs={12} style={{display:'flex', justifyContent:"center"}}>
                <Grid item sm={12} md={10} xs={10} id="reportCampain">
                  ...
                  {listData.map((el, index) => {
                    return (
                      <tr>
                        ...
                        <td align="left" style={{maxWidth: 300}}>{el && el.name  }</td>
...
// export default compose(withConnect)(ReportCampain);
