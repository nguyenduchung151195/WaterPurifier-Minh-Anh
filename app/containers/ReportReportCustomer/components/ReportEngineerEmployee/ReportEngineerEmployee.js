...
import { API_REPORT_ENGINEER_EMPLOYEE } from 'config/urlConfig';
...
function ReportEngineerEmployee(props) {
  ...
  useEffect(
    () => {
      if (months) {
        fetchData(`${API_REPORT_ENGINEER_EMPLOYEE}?month=${month}&year=${year}&${serialize(filter)}`)
          .then(res => {
  ...
  const handleCloseExcel = () => {
    ...
    let title = 'Báo cáo theo nhân viên kỹ thuật';
    switch (type) {
      case 'PDF':
        const { totalPage = 1, pageNumber = 1 } = {};
        const content = tableToPDF('reportByEngineerEmployee');
        setHtml(e => [...e, { content, pageNumber }]);
        setHtmlTotal(totalPage);
        break;
      default:
        tableToExcelCustomNoList('reportByEngineerEmployee', 'W3C Example Table', undefined, undefined, title);
    }
  };
    ...
    <>
      <Paper style={{ width: props.miniActive ? window.innerWidth - 80 : window.innerWidth - 260 }}>
        <Grid className="helloCVDA" style={{ padding: '0 20px', width: props.miniActive ? 'calc( 100vw - 80px)' : 'calc( 100vw - 260px)' }}>
          <CustomAppBar title=" BÁO CÁO THEO NHÂN VIÊN KỸ THUẬT" onGoBack={props.onClose} disableAdd />
          <Typography gutterBottom align="center" variant="h5" style={{ marginTop: 70 }}>
            BÁO CÁO THEO NHÂN VIÊN KỸ THUẬT
          </Typography>
          ...
          <Grid item md={12} container spacing={32} style={{ paddingTop: 30, paddingLeft: 50 }}>
            <Grid item xs={12} style={{ display: 'flex', justifyContent: 'center' }}>
              <Grid item sm={12} md={10} xs={10} id="reportByEngineerEmployee">
      ...
      <ExportTable
        ...
        // col={listYears}
        // listTitle={listTitle}
        titleExcel="Báo cáo theo nhân viên kỹ thuật"
        row={listData}
      />
    </>
  );
...
export default compose(withConnect)(ReportEngineerEmployee);
