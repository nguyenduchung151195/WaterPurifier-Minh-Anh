...
import GGEditor, { Flow, Minimap, ItemPanel, Item, withPropsAPI } from 'gg-editor';
...
// import makeSelectWorkFlow from './selectors';
...
export class WorkFlow extends React.Component {
  ...
  addNode(node) {
    if (node.addModel.type === 'node') {
      data.nodes.push(node.addModel);
      data.nodes[0].label = 'tt';
    }
    if (typeof node.addModel.target === 'string') data.edges.push(node.addModel);
  }
      ...
      <Card>
        ...
        >
          <Grid style={{ justifyContent: 'center', alignItems: 'center' }} container>
            ...
            <Grid style={{ background: '#2196f3' }} item md={8}>
              <Flow style={{ width: '100%', height: '500px' }} data={data} />
            </Grid>
            <Grid item md={2} />
            <Grid item md={8} />
            <Grid item md={4}>
              <Minimap style={{ width: 200, height: 200 }} />
            </Grid>
          </Grid>
        </GGEditor>
      </Card>
...
export default withPropsAPI(WorkFlow);
...
// const withReducer = injectReducer({ key: 'workFlow', reducer });
// const withSaga = injectSaga({ key: 'workFlow', saga });
