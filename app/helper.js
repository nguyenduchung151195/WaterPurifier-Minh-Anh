...
export function tableToPDF(id, code, fileName = 'PDF', title) {
  const sTable = document.getElementById(id) && document.getElementById(id).innerHTML;
...
async function apiCloneModule(data, moduleCode) {
  ...
  if (viewConfig) {
    const list = viewConfig.listDisplay ? viewConfig.listDisplay.type.fields.type.columns.filter(i => i.type.includes('Relation')) : null;
    list.forEach(i => {
      if (data[i.name]) {
...
export const taskTypeVoucher = ['Bán hàng', 'Mở rộng thị trường'];
export const taskTransport = ['Ô tô', 'Xe máy'];
export const taskTypeTransport = ['Thuê ngoài', 'Kh tự vận chuyển', 'MAW'];
...
export function tableToExcelCustomNoList(id, n, code, fileName = 'download', title) {
  if (title) {
    fileName = title;
  }
  const uri = 'data:application/vnd.ms-excel;base64,';
  ...
  function getHtml(table) {
    if (!table.nodeType) table = document.getElementById(table);
    table = table && table.innerHTML;
    table = table && table.split('<tfoot>');
    // table[0] = table[0].replaceAll('<td>', '<td>="')
    // table[0] = table[0].replaceAll('</td>', '"</td>')
    table = table && table.join('<tfoot>');
    return table;
  }
